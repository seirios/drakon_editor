...
proc get_context_inserts {} {
    # We use these as 'static' variables
    global _proc_get_context_inserts_insert
    global _proc_get_context_inserts_more
    global _proc_get_context_inserts_insert_eril
    global _proc_get_context_inserts_more_eril

    set diagram_id [ get_current_dia ]
    if { [ is_drakon $diagram_id ] } {
        if {![info exists _proc_get_context_inserts_insert]} {
            set result {}
            # 'Insert' menu
            ...
            set _proc_get_context_inserts_insert $result
        }
        if {![info exists _proc_get_context_inserts_more]} {
            set result {}
            # 'Insert more' menu
            #                     kind     icon_type  text                 accel
            lappend result [ list command  insertion  [ mc2 "Insertion" ]  "n" ]
            lappend result [ list command  shelf      [ mc2 "Shelf" ]      "f" ]
            lappend result { separator }
            lappend result [ list command  output     [ mc2 "Output" ]     "O" ]
            lappend result [ list command  input      [ mc2 "Input" ]      "I" ]
            lappend result { separator }
            lappend result [ list command  parallel   [ mc2 "Parallel" ]   "P" ]
            lappend result [ list command  process    [ mc2 "Process" ]    "R" ]
            lappend result [ list command  pause      [ mc2 "Pause" ]      "U" ]
            lappend result [ list command  timer      [ mc2 "Timer" ]      "T" ]
            set _proc_get_context_inserts_more $result
        }
        return [ list $_proc_get_context_inserts_insert $_proc_get_context_inserts_more ]
    } else {
        if {![info exists _proc_get_context_inserts_insert_eril]} {
            set result {}
            # 'Insert' menu
            #                     kind     icon_type   text                                 accel
            ...
            lappend result [ list command  commentin   [ mc2 "Inline comment" ]             "/" ]
            lappend result [ list command  commentout  [ mc2 "Standalone comment" ]         "C" ]
            set _proc_get_context_inserts_insert_eril $result
        }
        if {![info exists _proc_get_context_inserts_more_eril]} {
            set result {}
            # 'Insert more' menu
            #                     kind     icon_type         text                                    accel
            lappend result [ list command  up_white_arrow    [ mc2 "Inheritance link up" ]           "W" ]
            lappend result [ list command  left_white_arrow  [ mc2 "Inheritance link to the left" ]  "A" ]
            lappend result [ list command  down_white_arrow  [ mc2 "Inheritance link down" ]         "S" ]
            lappend result [ list command  right_white_arrow [ mc2 "Inheritance link to the right" ] "D" ]
            set _proc_get_context_inserts_more_eril $result
        }
        return [ list $_proc_get_context_inserts_insert_eril $_proc_get_context_inserts_more_eril ]
    }
}
